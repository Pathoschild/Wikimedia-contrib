    ...
    public function initWikiQueue(?array $defaultDbNames = null, int $minEdits = 1): bool
    {
        ...
        ########
            ...
            if (!$unifiedDbnames) {
                ...
                echo '<div id="result" class="neutral" data-is-error="1">', $this->formatText($this->username), ' has no global account, so we cannot auto-select an eligible wiki. Please select a wiki (see <a href="', $this->backend->getToolUrl('stalktoy', $this->username), '?defer=1" title="global details about this user" data-undefer>global details about this user</a>).</div>';
                return false;
            }
