...
class Script extends Base {
	...
	public $events = Array(
		39 => Array(
			'year' => 2016,
			'name' => 'Commons Picture of the Year for 2015',
			'url' => '//commons.wikimedia.org/wiki/Commons:Picture_of_the_Year/2015'
		),
	
		38 => Array(
			'year' => 2016,
			'name' => 'steward elections',
			'url' => '//meta.wikimedia.org/wiki/Stewards/Elections_2016'
		),
	...
	switch ($script->event['id']) {
			...
			do {
				...
				if(!$age_okay) {
					$age_okay = $script->condition(
						$date_okay = ($script->user['registration_raw'] < 20160101000000),
						"has an account registered before 01 January 2016 (registered {$script->user['registration']})...",
						"does not have an account registered before 01 January 2016 (registered {$script->user['registration']})."
					);
				}
				...
				########
				if(!$edits_okay) {
					$edits = $script->edit_count(NULL, 20160101000000);
		...
		case 38:
			...
			do {
				...
				########
				$edits = $script->edit_count(NULL, 20151101000000);
				$prior_edits += $edits;
				$script->condition(
					$edits >= 600,
				...
				########
				$edits = $script->edit_count(20150801000000, 20160131000000);
				$recent_edits += $edits;
				$script->condition(
					$edits >= 50,
		...
		case 37:
			...
			do {
				...
				########
				$script->condition(
					$script->user['registration_raw'] < 20150808000000,
					"has an account registered before 08 August 2015 (registered {$script->user['registration']})...",
					"does not have an account registered before 08 August 2015 (registered {$script->user['registration']})."
				);
				...
				if (!$minDurationMet) {
					/* check flag duration */
					$months = $script->get_role_longest_duration('sysop', 20160208000000);
