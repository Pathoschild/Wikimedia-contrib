...
###     <http://mootools.net/> (YUI compressed)
...
*****************/
function wmt_debug() {
	alert('WikiMooTools 1.2.4-r0');
}

/*****************
** Get edit token
*****************/
function wmt_token(args) {
	if(!args)
		args = {};
	var request = new Request({
		url: wgServer + '/w/api.php?format=json&action=query&prop=info&indexpageids=1&intoken=edit&titles=Whatever',
		method: 'get',
		onSuccess: function(responseText) {
			...
			if(args.onSuccess)
				...
				console.log('WikiMooTools> unhandled token (no onSuccess callback passed to wmt_token).');
		},
		onFailure: function(xhr) {
			if(args.onFailure)
				args.onFailure(xhr);
			else
				console.log('WikiMooTools> unhandled error: ' + xhr.status + ' ' + xhr.statusText + ' (no onFailure callback passed to wmt_token).');
		}
	});
	request.send();
}
...
/*****************
** Get array of page titles matching prefix
*****************/
function wmt_prefixindex(args) {
	/* set defaults */
	if(!args)
		args = {};
	if(!args.namespace)
		args.namespace = wgNamespaceNumber;
	if(!args.title)
		args.title = wgTitle;
	if(!args.onSuccess) {
		args.onSuccess = function(pages) {
			console.log('WikiMooTools> wmt_prefixindex unhandled success (' + pages.length + ' pages).');
		}
	}
	if(!args.onFailure) {
		args.onFailure = function(xhr) {
			console.log('WikiMooTools> wmt_prefixindex unhandled error (' + xhr.status + ' ' + xhr.statusText + ').');
		}
	}
	if(!args.passOn)
		args.passOn = null;
	...
	// subpages	
	var request = new Request({
		'url':wgServer+wgScriptPath+'/api.php?format=xml&action=query&list=allpages&apprefix=' + encodeURIComponent(args.title + (args.subpagesOnly?'/':'')) +'&aplimit=500&apnamespace=' + args.namespace,
		'method':'get',
		'onSuccess':function(responseText, responseXML) {
			// get pages
			var api_pages = responseXML.getElementsByTagName('p');
			var length = api_pages.length;
	
			// no pages
			if(length==0)
				args.onSuccess([], args.passOn);
			
			// push into array
			else {
				// output list items
				for(var i=0; i<length; i++)
					pages.push(api_pages[i].getAttribute('title'));
				
				// callback
				args.onSuccess(pages, args.passOn);
			}
		},
		'onFailure':function(xhr) { args.onFailure(xhr, args.passOn); }
	});
	request.send();
}

/*****************
** Returns a customizeable loading indicator
*****************/
function wmt_loader(args) {
	// defaults
	if(!args) args = {};
	if(!args.img) args.img = 'http://upload.wikimedia.org/wikipedia/commons/3/32/Loader3.gif';
	if(!args.txt) args.txt = 'Loading...';
	if(!args.container) args.container = new Element('span', {'class':'wmt_loader'});
	var box = args.container;
	
	// optional
	if(args.class) box.set('class', args.class + ' ' + box.get('class'));

	// put together
	box.adopt(new Element('img', {'src':args.img, 'styles':{'line-height':'0'}}));
	box.adopt(document.createTextNode(args.txt));
	return box;
}
// </source>
